...
//#define FULLMAPPING - addition for scam - define
/*
...
static void fillL3Info(l3pp_t l3) {
	...
	if (l3->l3info.setsperslice == 0)
		l3->l3info.setsperslice = (l3->cpuidInfo.cacheInfo.sets + 1)
				/ l3->l3info.slices;
	if (l3->l3info.bufsize == 0) {
		l3->l3info.bufsize = l3->l3info.associativity * l3->l3info.slices
				* l3->l3info.setsperslice * L3_CACHELINE * 2;
		if (l3->l3info.bufsize < 10 * 1024 * 1024)
			l3->l3info.bufsize = 10 * 1024 * 1024;
	}

//	printf("LLC Details:        \n");
//	printf("type            : %u\n", l3->cpuidInfo.cacheInfo.type);
//	printf("level           : %u\n", l3->cpuidInfo.cacheInfo.level);
//	printf("selfInitializing: %u\n", l3->cpuidInfo.cacheInfo.selfInitializing);
//	printf("fullyAssociative: %u\n", l3->cpuidInfo.cacheInfo.fullyAssociative);
//	printf("logIds          : %u\n", l3->cpuidInfo.cacheInfo.logIds + 1);
//	printf("phyIds          : %u\n", l3->cpuidInfo.cacheInfo.phyIds + 1);
//	printf("lineSize        : %u\n", l3->cpuidInfo.cacheInfo.lineSize + 1);
//	printf("partitions      : %u\n", l3->cpuidInfo.cacheInfo.partitions + 1);
//	printf("associativity   : %u\n", l3->cpuidInfo.cacheInfo.associativity + 1);
//	printf("sets            : %u\n", l3->cpuidInfo.cacheInfo.sets + 1);
//	printf("wbinvd          : %u\n", l3->cpuidInfo.cacheInfo.wbinvd);
//	printf("inclusive       : %u\n", l3->cpuidInfo.cacheInfo.inclusive);
//	printf("complexIndex    : %u\n", l3->cpuidInfo.cacheInfo.complexIndex);
...
#ifdef FULLMAPPING
	printf("Not Supported in Fullmapping mode");
...
void *sethead(l3pp_t l3, int set) { //vlist_t list, int count, int offset) {
	...
	offset = 0;
#else
	offset = (set % l3->groupsize) * L3_CACHELINE;
#endif

	for (int i = 0; i < count; i++) {

		LNEXT(OFFSET(vl_get(list, i), offset)) = OFFSET(
				vl_get(list, (i + 1) % count), offset);
		LNEXT(OFFSET(vl_get(list, i), offset+sizeof(void*))) = OFFSET(
				vl_get(list, (i + count - 1) % count), offset + sizeof(void *));
	}
...
static int probemap(l3pp_t l3) {
	if ((l3->l3info.flags & L3FLAG_NOPROBE) != 0)
		return 0;
	//printf("l3->l3info.bufsize=%d\n", l3->l3info.bufsize);

	int setsInRaw = l3->l3info.bufsize / 2 / l3->groupsize / L3_CACHELINE
			/ l3->l3info.associativity;
	//printf("setsInRaw=%d\n", setsInRaw);
	int groupsSize, RawSetsForMapping;
#ifdef FULLMAPPING
	groupsSize = l3->groupsize * setsInRaw;
	RawSetsForMapping = l3->groupsize;
#else
	groupsSize = setsInRaw;
	RawSetsForMapping = 1;
#endif // DEBUG

	l3->groups = (vlist_t *) calloc(groupsSize, sizeof(vlist_t));
	for (int j = 0; j < RawSetsForMapping; j++) {
		//printf("j=%d\n",j);
		vlist_t pages = vl_new();
		//printf("working dummy rawset %d\n", ((uint64_t)(l3->buffer+j*L3_CACHELINE)>>6)&0x3ff);
		for (int i = j * L3_CACHELINE; i < l3->l3info.bufsize;
				i += l3->groupsize * L3_CACHELINE)
			vl_push(pages, l3->buffer + i);
		vlist_t groups = map(l3, pages);
		if (vl_len(groups) != setsInRaw) {
//			printf("\trollback\n");
			j--;
			vl_free(groups);
			vl_free(pages);
		...
		printf("\rSets Mapped=%d", l3->ngroups);
		fflush(stdout);
		for (int i = 0; i < vl_len(groups); i++) {
#ifdef FULLMAPPING
			l3->groups[i * l3->groupsize + j] = vl_get(groups, i);
#else
...
static int ptemap(l3pp_t l3) {
	...
	for (int i = 0; i < l3->l3info.bufsize; i += l3->groupsize * L3_CACHELINE) {
		...
		vlist_t list = l3->groups[slice
				* (l3->l3info.setsperslice / l3->groupsize)
				+ cacheindex / l3->groupsize];
...
l3pp_t l3_prepare(l3info_t l3info) {
...
#ifndef HUGEPAGES
	printf("\n\nWarning - HUGEPAGES isnt defined\n\n\n\n\n")
#endif
	...
	l3->monitoredbitmap = (uint32_t *) calloc(l3->ngroups * l3->groupsize / 32,
			sizeof(uint32_t));
	l3->monitoredset = (int *) malloc(
			l3->ngroups * l3->groupsize * sizeof(int));
	l3->monitoredhead = (void **) malloc(
			l3->ngroups * l3->groupsize * sizeof(void *));
	l3->nmonitored = 0;
...
void l3_unmonitorall(l3pp_t l3) {
	...
	for (int i = 0; i < l3_getSets(l3) / 32; i++)
...
__always_inline void l3_probe(l3pp_t l3, uint16_t *results) {
	for (int i = 0; i < l3->nmonitored; i++) {
...
__always_inline void l3_bprobe(l3pp_t l3, uint16_t *results) {
	for (int i = 0; i < l3->nmonitored; i++) {
...
int l3_getSets(l3pp_t l3){
	return l3->cpuidInfo.cacheInfo.sets + 1;
}

// Returns the number of probed sets in the LLC
int l3_getMappedSets(l3pp_t l3) {
	int sets;
#ifdef FULLMAPPING
	sets = l3->ngroups;
#else
	sets=l3->ngroups*l3->groupsize;
#endif
	return sets;
}
